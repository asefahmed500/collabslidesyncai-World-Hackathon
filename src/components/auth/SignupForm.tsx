...
import { Zap, UserPlus, Mail, KeyRound, User as UserIcon } from "lucide-react"; // Removed Briefcase
...
const formSchema = z.object({
  ...
  // teamName: z.string().min(3, { message: "Team name must be at least 3 characters."}), // Team name removed for now
...
export function SignupForm() {
  ...
  useEffect(() => {
    if (formState.message) {
      if (formState.success) {
        ...
        setTimeout(() => {
          router.push("/login"); 
          router.refresh(); // Ensure useAuth hook picks up change if user is auto-logged in by Firebase
        }, 2500);
      } else {
        toast({
          title: "Signup Failed",
  ...
  const socialLogin = async (provider: GoogleAuthProvider | GithubAuthProvider) => {
    try {
      ...
      if (firebaseUser) {
        // After Firebase social sign-in, call server action to create/update MongoDB profile
  ...
  // const handleGitHubSignup = () => socialLogin(new GithubAuthProvider()); // GitHub can be added later
